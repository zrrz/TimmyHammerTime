    ...
    public class TileSpriteRenderer : TileRenderer
    {
        ...
        public override void UpdateTileAt(int x, int y)
        {
            ...
            if (current == null)
            {
				...
				current.name = string.Format("[{0}, {1}]", x, y) + ((current.GetComponent<Collider2D>() != null) ? "Col" : "No Col");
			}
			...
			if(tile == null) {
				if(current.GetComponent<Collider2D>() != null) {
					DestroyImmediate(current.GetComponent<Collider2D>());
				}
			}

            current.gameObject.SetActive(tile != null);
			current.gameObject.hideFlags = (tile != null) ? HideFlags.None : HideFlags.HideInHierarchy;
